...
public class GamePanel extends JPanel implements ActionListener, KeyListener {
    ...
    private long pauseStartTime = 0;           // 火球冷卻暫停起始
    private long pausedRemainingCd = 0;        // 火球冷卻暫停剩餘時間
    ...
    public GamePanel() {
        ...
        flashImg     = new ImageIcon(getClass().getResource("/resources/flash.png")).getImage();
        // 載入 Boss 圖片
        bossImgs[0] = new ImageIcon(getClass().getResource("/resources/boss1.png")).getImage();
        bossImgs[1] = new ImageIcon(getClass().getResource("/resources/boss2.png")).getImage();
        bossImgs[2] = new ImageIcon(getClass().getResource("/resources/boss3.png")).getImage();
        // 初始化 Boss 實例
            ...
            new Thread(() -> {
                try {
                    ...
                    c.open(AudioSystem.getAudioInputStream(
                            getClass().getResource("/resources/xf9c1-23hih.wav")
                    ));
                    c.start();
                } catch (Exception e) {
                    e.printStackTrace();
                ...
                if (f.getRect().intersects(e.rect)) {
                    ...
                    new Thread(() -> {
                        try {
                            Clip c = AudioSystem.getClip();
                            c.open(AudioSystem.getAudioInputStream(
                                    getClass().getResource("/resources/explosion.wav")
                            ));
                            c.start();
                        } catch (Exception ex) {
                            ex.printStackTrace();
...
// ── **新增：Boss3 子彈打到玩家** ──
    ...
    private abstract class Boss {
        ...
        protected void shootAtPlayer(double pX, double pY) {
            ...
            new Thread(() -> {
                try {
                    Clip c = AudioSystem.getClip();
                    c.open(AudioSystem.getAudioInputStream(getClass().getResource("/resources/xf9c1-23hih.wav")));
                    c.start();
                } catch (Exception ex) {
                    ex.printStackTrace();
                }
    ...
    private class Boss2 extends Boss {
        ...
        // 警告图与闪电图资源
        private final Image warningImg = new ImageIcon(getClass().getResource("/resources/warning.png")).getImage();
        private final Image flashImg   = new ImageIcon(getClass().getResource("/resources/flash.png")).getImage();
